...
```{r backup_data_kipling, eval=FALSE}
write_csv(twain_download, "twain_download.csv")
```

```{r backup_data_twain, eval=FALSE}
...
```{r relative_frequency_long_bravery}
...
```{r relative_frequency_long_friendship}
all_authors_frequency <- all_authors_tagged %>%
  group_by(title) %>%
  add_count(name = "length") %>%
...
```{r relative_frequency_long_courage}
all_authors_frequency <- all_authors_tagged %>%
  group_by(title) %>%
  add_count(name = "length") %>%
...
```{r relative_frequency_long_duty}
all_authors_frequency <- all_authors_tagged %>%
  group_by(title) %>%
  add_count(name = "length") %>%
...
```{r author_sentiment_table_bravery_friendship}
...
```{r author_sentiment_table_courage_duty}
author_sentiment_table <- author_sentiment_total %>%
